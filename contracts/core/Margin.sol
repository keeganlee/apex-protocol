...
contract Margin is IMargin, Reentrant {
    ...
    modifier onlyRouter() {
        require(IConfig(config).routerMap(msg.sender), "Margin: ONLY_ROUTER");
        _;
    }
    ...
    function addMargin(address trader, uint256 depositAmount) external override nonReentrant {
        require(IComptroller(comptroller).addMarginAllowed(address(this), depositAmount), "Margin.addMargin: NOT_ALLOW_TO_ADD_MARGIN");
    ...
    function openPosition(
        ...
        uint8 side,
        uint256 quoteAmount
    ) external override nonReentrant onlyRouter returns (uint256 baseAmount) {
